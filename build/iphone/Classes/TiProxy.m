...
//Rare exceptions to indicate a bug in the _kapflix code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_kapflix = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_kapflix]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.kapflix.module"
